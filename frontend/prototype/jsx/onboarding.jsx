// ============ Manthan — first-run onboarding ============

function ProviderKeyCard({ provider, compact }) {
  ...
  const submit = async () => {
    ...
    setEditing(false);
    await API.validateAndSaveKey(provider, draft);
    setDraft('');
  };
    ...
    <div className="card" style={{ padding: compact ? 16 : 20 }}>
      ...
      {status === 'valid' && !editing ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => API.validateAndSaveKey(provider, 'revalidate-' + k.masked)} title="Re-validate against the provider"><Icon name="retry" size={13} /></button>
            ...
            <div className="fade-up">
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 22 }}>
                ...
                <button className="btn btn-primary btn-lg" disabled={!state.defaultModel}
                  onClick={() => { API.completeOnboarding(); navigate('/home'); }}>
                  Enter Manthan
                </button>
              </div>
            </div>
...
Object.assign(window, { Onboarding, ProviderKeyCard });
