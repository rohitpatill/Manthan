...
import { useStore, Icon, ExpertAvatar, ModelChip, Md, fmtDate, ChatBubble, ManthanMark } from '../components/ui';
...
export function SessionCostTicker({ ses }) {
  useStore();
  const base = API.sessionCost(ses.id);
  // add a rough running cost for anything currently streaming
  let extra = 0, extraTok = 0;
  [1, 2].forEach((r) => {
    Object.entries(ses.rounds[r] || {}).forEach(([eid, resp]) => {
      if (resp.status === 'streaming') {
        const e = ses.experts.find((x) => x.id === Number(eid));
    ...
    const dm = API.getState().defaultModel;
    if (syn && syn.status === 'streaming' && dm) {
      const t = Catalog.estimateTokens(syn.text);
      extra += Catalog.costOf(dm.provider, dm.model, 0, t); extraTok += t;
    }
  });
...
function PhaseStepper({ ses }) {
  const phases = [
    ...
    ses.synthesis ? { key: 's1', label: 'Synthesis' } : null,
...
function ExpertResponseCard({ ses, round, expert, frozen }) {
  ...
  return (
    <div className="card fade-up" style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12, borderColor: r.status === 'failed' ? 'var(--red)' : undefined }}>
      ...
      {r.status === 'queued' && !frozen ? (
        <div style={{ color: 'var(--ink-3)', fontSize: 12.5, padding: '6px 0' }}>Waiting to dispatch…</div>
      ) : null}

      {showBody ? (
        <div>
          <div style={{ maxHeight: expanded || !long ? 'none' : 215, overflow: 'hidden', position: 'relative' }}>
            <Md text={r.text} />
          ...
          {long ? (
            <button className="btn btn-subtle btn-sm" style={{ marginTop: 6, color: 'var(--primary)' }} onClick={() => setExpanded(!expanded)}>
              {expanded ? 'Collapse' : 'Read full response'} <Icon name="down" size={12} style={{ transform: expanded ? 'rotate(180deg)' : 'none' }} />
            </button>
          ) : null}
        </div>
      ) : null}
      ...
      {r.status === 'failed' ? (
        <div style={{ background: 'var(--red-tint)', borderRadius: 9, padding: '11px 13px' }}>
          ...
          {!frozen ? (
            <button className="btn btn-ghost btn-sm" disabled={API.isRunning(ses.id)} onClick={() => API.retryExpert(ses.id, round, expert.id)}>
      ...
      {r.usage && r.status === 'done' ? (
        <div style={{ fontSize: 11, color: 'var(--ink-3)', display: 'flex', gap: 12 }} className="mono">
          ...
          <span>{Catalog.fmtCost(r.usage.cost != null ? r.usage.cost : Catalog.costOf(expert.provider, expert.model, r.usage.inTok, r.usage.outTok))}</span>
        </div>
      ) : null}
    </div>
  );
  ...
  return (
    <section style={{ marginBottom: 28 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: 14 }}>
        {ses.experts.map((e) => <ExpertResponseCard key={e.id} ses={ses} round={round} expert={e} frozen={frozen} />)}
      </div>
    </section>
  );
}
...
// ---------- synthesis ----------
function SynthesisCard({ ses, round }) {
  ...
  return (
    <section className="fade-up" style={{ marginBottom: 28 }}>
      <div className="card" style={{ borderColor: 'var(--gold-line)', background: 'var(--gold-tint)', padding: 24 }}>
        ...
        {syn.stance ? (
          <div style={{ background: '#fff', border: '1px solid var(--gold-line)', borderRadius: 9, padding: '9px 12px', fontSize: 12.5, fontWeight: 700, color: 'var(--gold-deep)', lineHeight: 1.5, marginBottom: 12 }}>
            {syn.stance}
          </div>
        ) : null}
        {syn.status === 'thinking' ? (
          ...
          <p style={{ fontSize: 13, color: 'var(--red)' }}><Icon name="warn" size={13} /> Synthesis failed: {syn.error}</p>
        ) : (
          <div>
            <Md text={syn.text} />
...
function RunBanners({ ses }) {
  ...
  if (ses.runError && !running) {
    return (
      <div className="card fade-up" style={{ padding: '14px 18px', marginBottom: 20, borderColor: 'var(--red)', display: 'flex', alignItems: 'center', gap: 12 }}>
        <Icon name="warn" size={17} style={{ color: 'var(--red)', flex: 'none' }} />
        <p style={{ flex: 1, fontSize: 13, color: 'var(--ink-2)' }}><strong>Run error:</strong> {ses.runError}</p>
        <button className="btn btn-primary btn-sm" onClick={() => API.resume(ses.id)}><Icon name="retry" size={13} /> Resume</button>
      </div>
    );
  }
  if (partial && !running) {
    ...
    return (
      <div className="card fade-up" style={{ padding: '14px 18px', marginBottom: 20, borderColor: 'var(--red)', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          <p style={{ fontWeight: 700, fontSize: 13.5 }}>{failed.length} of {ses.experts.length} expert calls failed in Round {round}.</p>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-primary btn-sm" onClick={async () => {
            for (const [eid] of failed) await API.retryExpert(ses.id, round, Number(eid));
...
const PURPOSE_LABEL = {
  intake: 'Intake (Manthan AI)', expert_r1: 'Expert — Round 1', expert_r2: 'Expert — Round 2',
  synthesis_r1: 'Synthesis (Manthan AI)', synthesis_r2: 'Final synthesis (Manthan AI)',
};
...
export function FrozenView({ ses }) {
  ...
  return (
    <div style={{ maxWidth: 1180, margin: '0 auto', padding: '24px 32px 64px' }}>
      ...
      <Collapsible title={ROUND_META[1].title} sub={ses.experts.length + ' experts'} defaultOpen>
        <SummaryStrip ses={ses} round={1} />
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: 14 }}>
          {ses.experts.map((e) => <ExpertResponseCard key={e.id} ses={ses} round={1} expert={e} frozen />)}
        </div>
      </Collapsible>

      {ses.syntheses[1] ? (
        <Collapsible title={ses.round2 ? 'Synthesis of Round 1' : 'Final synthesis'} defaultOpen={!ses.round2}>
          <SynthesisCard ses={ses} round={1} />
        </Collapsible>
      ) : null}

      {Object.keys(ses.rounds[2] || {}).length ? (
        <Collapsible title={ROUND_META[2].title} sub={ses.experts.length + ' experts'}>
          <SummaryStrip ses={ses} round={2} />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: 14 }}>
            {ses.experts.map((e) => <ExpertResponseCard key={e.id} ses={ses} round={2} expert={e} frozen />)}
          </div>
        </Collapsible>
      ) : null}
            ...
            {ses.usageLog.map((u) => (
              <tr key={u.id} style={{ borderTop: '1px solid var(--line-2)' }}>
                <td style={{ padding: '7px 8px', fontWeight: 600 }}>
                  {u.purpose.startsWith('expert') ? u.expert_name : (PURPOSE_LABEL[u.purpose] || u.purpose)}
                  {u.purpose === 'expert_r2' ? <span style={{ color: 'var(--ink-3)', fontWeight: 400 }}> · R2</span> : null}
                </td>
                <td style={{ padding: '7px 8px', color: 'var(--ink-3)' }}>{u.model_id}</td>
                <td style={{ padding: '7px 8px', textAlign: 'right' }}>{Catalog.fmtTok(u.input_tokens)}</td>
                <td style={{ padding: '7px 8px', textAlign: 'right' }}>{Catalog.fmtTok(u.output_tokens)}</td>
                <td style={{ padding: '7px 8px', textAlign: 'right', color: 'var(--ink-3)' }}>{u.cached_tokens ? Catalog.fmtTok(u.cached_tokens) : '—'}</td>
                <td style={{ padding: '7px 8px', textAlign: 'right', fontWeight: 700 }}>{Catalog.fmtCost(u.cost)}</td>
              </tr>
            ))}
          </tbody>
        </table>
