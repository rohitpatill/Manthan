...
// ---- Manthan mark: nested churn arcs ----
export function ManthanMark({ size = 26, color = 'currentColor' }) {
  return (
...
export function ExpertAvatar({ expert, size = 36, style }) {
  const [broken, setBroken] = useState(false);
  useEffect(() => setBroken(false), [expert && expert.avatar && expert.avatar.value]);
...
export function ModelChip({ provider, model, invalid }) {
...
export function ModelPicker({ value, onChange, style }) {
  useStore();
...
// ---- modal ----
export function Modal({ width = 560, onClose, children, dismissable = true }) {
  useEffect(() => {
  ...
  return (
    <Modal width={420} onClose={onClose}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '20px 24px 22px' }}>
        ...
        <button className="btn btn-primary" style={danger ? { background: 'var(--red)' } : null}
...
export function EditableTitle({ value, onChange, style }) {
...
export function SessionStatus({ status }) {
  ...
  if (status === 'awaiting-approval' || status === 'ready') return <span className="badge badge-amber">Brief ready</span>;
  if (status && String(status).includes('partial')) return <span className="badge badge-red">Needs attention</span>;
